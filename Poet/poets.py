...
def run():
    ...
    with open("Poets.txt", "w",encoding="utf-8") as f:
        url=f"https://www.gushimi.org/shiren/index.html"
        Bing_response = requests.get(url, proxies=p, headers=headers)
        soup = Bs(Bing_response.text, "html.parser")
        l=soup.find_all("div",class_="news_title")
        for i in l:
            f.write(i.text.encode("ISO-8859-1").decode("utf-8")+"\n")
        for i in range(2,281):
            url=f"https://www.gushimi.org/shiren/index_{i}.html"
            Bing_response = requests.get(url, proxies=p, headers=headers)
            soup = Bs(Bing_response.text, "html.parser")
            l=soup.find_all("div",class_="news_title")
            for item in l:
                f.write(item.text.encode("ISO-8859-1").decode("utf-8")+"\n")
